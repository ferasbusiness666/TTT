...
- [~] **Sign Cloudinary uploads for signed-in staff** (#5) — closes anonymous upload abuse. Built: `/api/sign-upload` signs uploads for authenticated staff, and the editor prefers it, falling back to the unsigned preset so uploads can never break. **Not closed until the `ttt-posts` preset is switched to Signed (or deleted) in the Cloudinary dashboard** — a manual step, and only worth doing once you've confirmed a photo still uploads through the signed path.
