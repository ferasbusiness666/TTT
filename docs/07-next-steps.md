...
- [ ] Decide how to handle **orphaned Cloudinary images** when a post is deleted — periodic cleanup, or a Cloudflare Pages Function holding the API secret. See `05-known-issues.md` for why the browser can't do it. Not urgent while the library is tiny.
