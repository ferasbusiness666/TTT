...
- [~] **Admin desk cleanup** — remove the **Media** and **Comments** sidebar items (done), and either fix the **Settings** gear icon (its SVG path is malformed, so it renders as a broken shape) or drop Settings entirely. **ASK:** remove or fix? Fero leans remove, since there's nothing there to change.
...
- [ ] **Library/gallery shows every post's image** — today the gallery section only pulls posts of type `artwork`, and only fills when there are 8 of them. It should show the images from **all** posts, each linking to the post it belongs to. **ASK:** cover images only, or in-body photos too (one post could then appear several times)?
- [ ] **Founders section: credit line for Fero** — not as a founder; a separate line below them naming him as the person who designed and built the site, with an email for support, bug reports and contact. **ASK:** which email address to publish.
...
- [ ] **Trending order is wrong** — confirmed bug, not a matter of taste. `.k-featured` is the big card (2 columns × 2 rows), but it sits at **index 1** of `BENTO_SLOTS` in `index.html`, so the *newest* post lands in a small top-left card and the second-newest gets the hero slot. Also worth a pass on how real posts actually look once published. **ASK:** fix by moving the featured slot to the front of the array (also changes mobile order, where the grid stacks in DOM order), or keep the layout byte-identical and just assign the newest post to the featured slot?
