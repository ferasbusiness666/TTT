...
## Open questions
- **Topical category picker (needs a design decision).** The editor has one dropdown that mixes content *type* (Articles/Videos/Artwork/Magazines) with a couple of topic labels (Opinion/Review). Post-creation currently maps it to `post_type` only, leaving `category_id` null. A real topical taxonomy (Opinion, News, Culture, Review, Photo Essay, Tech, Special Issue — per `03-features-and-content-model.md`) needs the `categories` table seeded **and** a second control in the editor to pick one. Adding that control is a visual change, so raise it with Fero before building. Until then, posts have a type but no category.
