...
## Mobile UI logged as a blocker
- Fero reported the phone layout is broken (screenshots): clipped wordmark, missing search control, dead space above the category tabs, and the header misbehaving on scroll. Written up with specifics in `05-known-issues.md` and added to `07-next-steps.md`. Not fixed yet — logged as the next visual task.
